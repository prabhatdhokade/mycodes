...
wti, we need to calculate minimum amount that could make up this quantity exactly.
...
int solve(int val[],int wt[],int W,int n) {
    ...
    for(int i =1;i<n+1;i++){
        for(int j =1;j<W+1;j++){
            ...
            if(wt[i-1] <= j ) {
                t[i][j] = max(val[i-1] + t[i] [j-wt[i-1]],  // THIS IS THE DIFFERENCE t[i] rather than t[i-1];
                            t[i-1][j] );
            }
            /* 
            else if (wt[n-1] > W) {
